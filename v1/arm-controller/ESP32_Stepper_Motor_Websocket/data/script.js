...
function submitForm(){
    const rbs = document.getElementById("direction");
    direction = rbs.value;
    ...
    websocket.send(axis+"&"+steps+"&"+direction);
}


function updateArmPos(axis, steps, direction)
{
    console.log(axis + steps + direction);
    websocket.send(axis+"&"+steps+"&"+direction);
}
